...
int main()
{
    ...
    while (t--)
    {
        ...
        init(n);
        ll ans = _max(query(0, n - 2, 0, 0, _n).rms, query(1, n - 1, 0, 0, _n).rms);
        printf(sum > ans ? "YES\n" : "NO\n");
    }
    return 0;
}
...
void init(const int n)
{
    ...
    for (int i = _n - 1; i >= 0; --i)
    {
        ...
        cur.left = _max(ls.left, ls.all + rs.left);
        cur.right = _max(rs.right, rs.all + ls.right);
        cur.rms = _max(_max(ls.rms, rs.rms), ls.right + rs.left);
    }
}
...
const P query(const int &left, const int &right, int k, int begin, int end)
{
    ...
    else if (left <= end && begin <= right)
    {
        ...
        return P(_max(ls.left, ls.all + rs.left), _max(rs.right, rs.all + ls.right), ls.all + rs.all,
                 _max(_max(ls.rms, rs.rms), ls.right + rs.left));
    }
    else
    {
        return P();
